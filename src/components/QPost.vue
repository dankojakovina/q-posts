        ...
        <div class="col-sm-12 col-md-8">
          <div class="row">
            <div class="col-sm-6 col-6">
              <div class="small font-italic date-color  mr-2">
                10. JUL 2022
              </div>
            </div>
          ...
          <div class="row">
            <div class="col-sm-12">
              <div class="text-dark font-weight-400 ft-size-16 ln-height-16 mb-3">
                {{ post.user.name }}
                <div class="small text-dark font-italic">
          ...
          <div class="row">
            <div class="col-sm-12">
              <div
                class="ln-height-16 mb-2 text-dark font-weight-bold text--dark ft-size-16 pointer underline-hover"
                @click="goToDetails">
                {{ post.title }}
              </div>
            </div>
          ...
          <div class="row">
            <div class="col-sm-12">
              <div class="post-text text-dark ln-height-18">
                {{ post.body }}
              </div>
            </div>
          </div>
          ...
          <div
            ...
            <div class="col-sm-12">
              <div
                ...
                class="small pointer"
                @click="toggleReplays">
                <span class="text-light">{{ $t('see') }} <span v-if="isPlural">{{ $t('all')
                }}</span></span> <span class="text-dark font-weight-bold"> <span v-if="isPlural">{{ $t('comments')
                                                                           }}</span>
                  <span v-else>{{ $t('comment') }}</span>( {{ post.comments.length }} ) </span>
              </div>
              <div
                v-else
                class="small pointer text-light"
                @click="toggleReplays">
                {{ $t('hide_comments')
                }}
              </div>
